...
On the pinned corpus a projection that reproduces the annotation exactly scores 1.0 on every dimension. Reversing the blocks, and replacing every block with empty or unrelated text, score strictly below that; merging every text block into one paragraph now costs block structure alone, which is the dimension that question belongs to. Eleven degraded variants were swept across all six dimensions on all 981 pages and every one scored at or below exact reproduction. That sweep predates both the identification term and the split, and each of its variants deleted blocks, so the variants that keep the block structure and destroy the content — and the ones that keep both and only re-chunk — are pinned as unit tests instead. Both the sweep and its per-variant figures are re-measured on the run that bootstraps the baseline.
