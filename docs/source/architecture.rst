...
.. code-block:: text
   ...
   │                                                             │
   │  • Registers built-ins from the manifest                  │
...
* **Built-in registration:** Registers built-in parsers/renderers from the generated ``_converter_manifest`` module (regenerated by ``scripts/generate_converter_manifest.py``)
...
* **Extensibility:** Third-party packages can register converters via the ``all2md.converters`` entry-point group
...
.. code-block:: python
   ...
   class ConverterRegistry:
       def auto_discover(self):
           """Register built-ins from the generated manifest, then discover plugins"""

       def register(self, metadata):
           """Register a converter from its ConverterMetadata"""

       def get_parser(self, format_name):
           """Get parser class for format"""

       def get_renderer(self, format_name):
           """Get renderer class for format"""

       def get_parser_options_class(self, format_name):
           """Get options dataclass for format"""

**Entry Point Example (third-party plugins):**

Built-in converters are registered from the generated manifest. External packages add
their own converters through the ``all2md.converters`` entry-point group:

.. code-block:: toml

   # pyproject.toml of a third-party plugin package
   [project.entry-points."all2md.converters"]
   mydocs = "my_all2md_plugin:CONVERTER_METADATA"
