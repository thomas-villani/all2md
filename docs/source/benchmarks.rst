...
Ground truth is the hard part of measuring conversion. all2md uses four independent
...
.. list-table::
   ...
   * - ``docx``
     - Word documents generated by scripting a live Word
     - The facts the generating script wrote beside each file
     - Documents nobody scripted — wild templates, other writers' markup
...
born-digital path and almost never OCR. Neither substitutes for the other. A fifth lane,
:file:`benchmarks/comparison/`, scores third-party converters with the PMC lane's own
instruments; it is run by hand and never in CI.
...
Two further instruments sit beside that reading. ``benchmarks/comparison/headings.py``
scores section headings across every tool by exact match against an emitted heading —
all three lose about a fifth of printed headings into the prose stream, within a point of
each other — and ``benchmarks/comparison/tablediag.py`` splits the table gap into content
and order: all2md holds 99.8% of every attainable table's words, so what remains is row
grouping, a difference between a learned table model and geometry rules rather than a
missing rule. ``python -m benchmarks.pmc.audit`` checks the ground truth itself, and its
first run found that the published ``title`` figure is mostly reference-list entries,
because a scoring floor never sees two-word headings.
...
The ``docx`` lane exists because there is no public bucket of Word files with independent
structural truth beside them, the way PMC pairs a PDF with its JATS. So the truth is
generated: a case script drives a live Word over COM (through the sister project
``wordlive``), and because the script knows exactly what it put in the document it writes an
expected-facts record beside it, while Word's own serializer produces the file. Truth and
artifact come from different tools, which is what makes the PMC lane trustworthy, and it buys
something a collected corpus never can — paired documents identical but for one construct.

Generation cannot run in CI, so the corpus bytes are committed and digest-pinned, and replay
costs seconds. At this release the corpus holds 23 cases across families for tracked
changes, comments, notes, numbering, fields, content controls, tables and styles, several of
them controls. The lane gates every pull request on three things: nothing crashes, no control
case reports a defect, and scoring cannot silently produce no findings. The defect *count* is
deliberately not gated, because a gate on it would punish adding a case that exposes a real
defect. Its first run found silent total losses — tracked insertions, display equations,
style-inherited numbering, content controls, field results — that the round-trip score could
never see, since a parser and a renderer that agree on a wrong reading score perfectly.
...
and the Word lane's crash and control checks are the exceptions, and OmniDocBench ratchets
against a recorded baseline monthly.
...
.. code-block:: bash
   ...
   python -m benchmarks.docx                   # the Word-generated corpus, replayed from committed bytes
