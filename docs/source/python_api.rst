...
- Automatically detects the source format when possible and infers the target format from the
  ``output`` name.
- Accepts file paths, in-memory strings/bytes, or file-like objects.
...
3. **Handle Rendering Errors Gracefully**
   ...
   .. code-block:: python
      ...
      try:
          convert('document.md', target_format='pdf', output='output.pdf')
...
.. note::
   ...
   :doc:`templates` is the canonical reference for the Jinja2 renderer — the
   full template context, filters, helper functions, and feature flags live
   there. This section is a Python-API-focused summary; if anything here and
   the template guide ever disagree, the template guide is authoritative.
