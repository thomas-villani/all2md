...
### Added
- **Creator metadata for rendered documents**: All document renderers now embed "all2md" as the creator/producer application in output files. Supported formats: DOCX, PPTX, PDF, EPUB, ODT, ODP. Configurable via `--renderer-creator` CLI flag or `creator` option in renderer options. Set to empty string to disable.
