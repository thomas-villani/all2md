...
- **DOCX: footnotes and endnotes keep their formatting, tabs, links and lists.** Every
  note paragraph was read as plain text, so `**bold**` in a note printed as `bold`, and
  the space Word puts after the note number printed as a double space (`[^1]:  text`).
  Tracked changes, content controls and fields inside notes are now resolved too: before,
  `revisions="reject"` still printed text inserted in a note, and `revisions="mark"` did
  not mark it.
- **DOCX: a numbered list that runs from one footnote into the next keeps counting.** It
  restarted at `1.` in the second note where Word prints `3.`, `4.`. Footnotes and
  endnotes count their lists separately from the document body, as Word does.
...
- **DOCX: numbering prints the labels Word shows.** A numbered heading reads
  `## 1.1 Definitions` or `# Article I Definitions` instead of losing its number. A clause
  labelled `(a)`, `1.1` or `03)`, which Markdown list syntax cannot write, becomes a
  paragraph starting with that label, while plain `1.` lists stay Markdown lists. Letter,
  Roman, zero-padded and legal (`w:isLgl`) numbering are all written as Word prints them.
  A list followed directly by a heading is no longer emitted after the heading. The
  Markdown renderer now escapes a paragraph that begins like an ordered list marker
...
- **Tests: external PDF fidelity against pinned OmniDocBench ground truth.**
  ...
  baseline is recorded: text content 0.5058, reading order 0.6034, block structure 0.1176
  over all 981 pages.
  Exact reproduction of the annotation scores 1.0, and no degradation of it scores higher:
  checked across eleven degraded variants and all six dimensions that then existed, on all
  981 pages, plus the
  ...
  segmentation is a real question and a different one from ordering. Read that dimension as a
  granularity ratio rather than a quality score: it is bounded by the ratio of the two block
