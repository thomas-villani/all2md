...
#
# src/all2md/options/json.py
"""Options for JSON parsing and rendering.

This module defines configuration options for converting between JSON and AST.
The parser converts JSON structures to readable document format, while the
renderer extracts tables and structured data from documents back to JSON.
"""
...
class JsonParserOptions(BaseParserOptions):
    ...
    include_type_hints : bool, default = False
        If True, add metadata hints about the original JSON types (useful for
    ...
    include_type_hints: bool = field(
        default=False,
        metadata={"help": "Add metadata hints about original JSON types", "importance": "advanced"},
    )
...
@dataclass(frozen=True)
class JsonRendererOptions(BaseRendererOptions):
    ...
    ensure_ascii : bool, default = False
        If True, escape non-ASCII characters in output.
    sort_keys : bool, default = False
        If True, sort keys in output JSON.
    ...
    indent: int | None = field(
        default=2,
        metadata={
            "help": "Number of spaces for JSON indentation (None for compact)",
            "type": int,
            "importance": "core",
        },
    )
    ensure_ascii: bool = field(
        default=False,
        metadata={"help": "Escape non-ASCII characters in output", "importance": "advanced"},
    )
    sort_keys: bool = field(
        default=False,
        metadata={"help": "Sort keys in output JSON", "importance": "core"},
    )
