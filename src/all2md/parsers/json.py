...
#
# src/all2md/parsers/json.py
"""JSON to AST converter.

This module provides intelligent conversion from JSON structures to readable
document format. By default, it transforms the structure into a hierarchical
document with headings, tables, and lists. Alternatively, it can render JSON
...
class JsonParser(BaseParser):
    """Convert JSON structures to AST representation.

    This parser creates a readable document from JSON by converting:
    ...
    Parameters
    ----------
    options : JsonParserOptions or None, default = None
    ...
    With options:
        ...
        >>> options = JsonParserOptions(max_heading_depth=3, sort_keys=True)
        >>> parser = JsonParser(options)
        >>> doc = parser.parse(json_file_path)

    """

    def __init__(self, options: JsonParserOptions | None = None, progress_callback: Optional[ProgressCallback] = None):
        """Initialize the JSON parser with options and progress callback."""
        BaseParser._validate_options_type(options, JsonParserOptions, "json")
        options = options or JsonParserOptions()
        super().__init__(options, progress_callback)
        self.options: JsonParserOptions = options
    ...
    def parse(self, input_data: Union[str, Path, IO[bytes], bytes]) -> Document:
        ...
        """
        self._emit_progress("started", "Parsing JSON", current=0, total=100)
        ...
        try:
            ...
            if self.options.literal_block:
                self._emit_progress("completed", "JSON wrapped as code block", current=100, total=100)

                # Optionally pretty-print the JSON
                try:
                    data = json.loads(content)
                    # Re-serialize with formatting
                    formatted_json = json.dumps(data, indent=2, ensure_ascii=False, sort_keys=self.options.sort_keys)
                except json.JSONDecodeError:
                    # If invalid JSON, just use the raw content
                    formatted_json = content

                # Create code block
                code_block = CodeBlock(content=formatted_json, language="json")
                metadata = DocumentMetadata()
                return Document(children=[code_block], metadata=metadata.to_dict())

            self._emit_progress("parsing", "Parsing JSON structure", current=30, total=100)
            ...
            self._emit_progress("completed", "JSON parsing completed", current=100, total=100)
    ...
    def _convert_value(self, value: Any, depth: int, key: str | None = None) -> list[Node]:
        """Convert a JSON value to AST nodes.
    ...
    def _convert_object(self, obj: dict[str, Any], depth: int) -> list[Node]:
        """Convert a JSON object to AST nodes.
        ...
        all_primitives = all(isinstance(obj[k], (str, int, float, bool, type(None))) for k in keys)
    ...
    def _convert_array(self, arr: list[Any], depth: int, key: str | None = None) -> list[Node]:
        """Convert a JSON array to AST nodes.
...
def _detect_json_content(content: bytes) -> bool:
...
CONVERTER_METADATA = ConverterMetadata(
    ...
    magic_bytes=[],
    content_detector=_detect_json_content,
