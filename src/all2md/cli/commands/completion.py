...
def generate_bash_completion(catalog: dict[str, Any]) -> str:
    ...
    # Get available extensions (those with satisfied dependencies)
    available_extensions = catalog.get("available_extensions", [])
...
# Supported file extensions (based on installed dependencies)
_all2md_supported_extensions=({' '.join(f'"{ext}"' for ext in available_extensions)})
...
"""
    ...
    # File completion filtered by supported extensions
    # First, add directory completions for navigation
    local IFS=$'\\n'
    local dirs
    ...
    # Then add file completions filtered by supported extensions
    local ext
    for ext in "${_all2md_supported_extensions[@]}"; do
        local files
        files=$(compgen -f -X "!*$ext" -- "$cur" 2>/dev/null)
        if [[ -n "$files" ]]; then
            while read -r file; do
                # Only add if it's a file, not a directory
                [[ -f "$file" ]] && COMPREPLY+=("$file")
            done <<< "$files"
        fi
    done
    return 0
...
def generate_zsh_completion(catalog: dict[str, Any]) -> str:
    ...
    # Build glob pattern for zsh _files (e.g., "*.pdf *.docx *.html")
    extensions_glob = " ".join(f"*{ext}" for ext in available_extensions)
...
# Supported file extensions (based on installed dependencies)
_all2md_extensions=({extensions_glob})

# Custom file completer that filters by supported extensions
_all2md_files() {{
    # Complete directories for navigation
    _path_files -/
    # Complete files with supported extensions
    _files -g "${{_all2md_extensions}}"
}}
...
def generate_powershell_completion(catalog: dict[str, Any]) -> str:
    ...
    # Build extensions array for PowerShell
    extensions_array = ", ".join(f"'{ext}'" for ext in available_extensions)

    # Generate PowerShell script
    script = f"""# PowerShell completion for all2md
...
Register-ArgumentCompleter -CommandName all2md -ScriptBlock {{
    ...
    # Supported file extensions (based on installed dependencies)
    $supportedExtensions = @({extensions_array})
...
"""
    ...
    # Extract parent path prefix from wordToComplete to preserve directory context
    $parentPath = if ($wordToComplete -match '[\\/]') {
    ...
    $paths = Get-ChildItem -Path "$wordToComplete*" -ErrorAction SilentlyContinue |
        ...
        ForEach-Object {
            $completionText = if ($_.PSIsContainer) { $parentPath + $_.Name + '\' } else { $parentPath + $_.Name }
