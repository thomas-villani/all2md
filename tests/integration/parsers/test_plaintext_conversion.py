...
class TestPlainTextFormatStripping:
    ...
    def test_strip_heading_markers(self) -> None:
        ...
        doc = Document(
            children=[
                Heading(level=1, content=[Text(content="Title")]),
                Heading(level=2, content=[Text(content="Section with "), Strong(content=[Text(content="bold")])]),
            ]
        )
        ...
        doc = Document(
            children=[
                Paragraph(
                    content=[
                        Text(content="Visit "),
                        Link(content=[Text(content="our website")], url="https://example.com"),
...
class TestPlainTextCodeBlocks:
    ...
    def test_inline_code(self) -> None:
        ...
        doc = Document(
            children=[
                Paragraph(content=[Text(content="Use the "), Code(content="print()"), Text(content=" function.")])
            ]
        )
...
class TestPlainTextOptions:
    ...
    def test_all_options_combined(self) -> None:
        ...
        options = PlainTextOptions(
            paragraph_separator="\n\n", list_item_prefix=">> ", table_cell_separator=" | ", max_line_width=None
        )
        renderer = PlainTextRenderer(options=options)
        result = renderer.render_to_string(doc)
...
class TestPlainTextEdgeCases:
    ...
    def test_nested_formatting(self) -> None:
        ...
        doc = Document(children=[Paragraph(content=[Strong(content=[Emphasis(content=[Code(content="nested")])])])])
    ...
    def test_block_quote_extraction(self) -> None:
        ...
        doc = Document(children=[BlockQuote(children=[Paragraph(content=[Text(content="Quoted text here.")])])])
    ...
    def test_image_alt_text(self) -> None:
        ...
        doc = Document(
            children=[
                Paragraph(
                    content=[Text(content="See "), Image(url="photo.jpg", alt_text="the photo"), Text(content=" here.")]
                )
            ]
        )
    ...
    def test_complex_nested_structure(self) -> None:
        ...
        doc = Document(
            children=[
                ...
                List(
                    ordered=False,
                    items=[ListItem(children=[Paragraph(content=[Text(content="Item with "), Code(content="code")])])],
                ),
                Table(
                    header=TableRow(cells=[TableCell(content=[Strong(content=[Text(content="Header")])])]),
                    rows=[TableRow(cells=[TableCell(content=[Emphasis(content=[Text(content="Cell")])])])],
                ),
            ]
        )
