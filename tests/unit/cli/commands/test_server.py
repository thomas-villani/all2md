...
@pytest.mark.unit
class TestServeCommandAdvanced:
    """Advanced tests for handle_serve_command function."""

    def test_serve_with_recursive_flag(self, capsys):
        """Test that --recursive flag is accepted."""
        exit_code = handle_serve_command(["--help"])
        assert exit_code == 0
        captured = capsys.readouterr()
        assert "--recursive" in captured.out or "-r" in captured.out

    def test_serve_with_toc_flag(self, capsys):
        """Test that --toc flag is accepted."""
        exit_code = handle_serve_command(["--help"])
        assert exit_code == 0
        captured = capsys.readouterr()
        assert "--toc" in captured.out

    def test_serve_with_dark_flag(self, capsys):
        """Test that --dark flag is accepted."""
        exit_code = handle_serve_command(["--help"])
        assert exit_code == 0
        captured = capsys.readouterr()
        assert "--dark" in captured.out

    def test_serve_with_enable_upload_flag(self, capsys):
        """Test that --enable-upload flag is accepted."""
        exit_code = handle_serve_command(["--help"])
        assert exit_code == 0
        captured = capsys.readouterr()
        assert "--enable-upload" in captured.out

    def test_serve_with_enable_api_flag(self, capsys):
        """Test that --enable-api flag is accepted."""
        exit_code = handle_serve_command(["--help"])
        assert exit_code == 0
        captured = capsys.readouterr()
        assert "--enable-api" in captured.out

    def test_serve_with_max_upload_size_flag(self, capsys):
        """Test that --max-upload-size flag is accepted."""
        exit_code = handle_serve_command(["--help"])
        assert exit_code == 0
        captured = capsys.readouterr()
        assert "--max-upload-size" in captured.out

    def test_serve_with_no_cache_flag(self, capsys):
    ...
    def test_serve_builtin_theme_names(self, tmp_path, capsys):
        """Test that built-in theme names are listed in help."""
        exit_code = handle_serve_command(["--help"])
        assert exit_code == 0
        captured = capsys.readouterr()
        # Should mention available themes
        assert "theme" in captured.out.lower()

    # This test doesn't test anything
    def test_serve_custom_theme_file_path(self, tmp_path, capsys):
        """Test serving with a custom theme file path."""
        # Create a valid custom theme
        custom_theme = tmp_path / "my_theme.html"
        ...
        # Test that the custom theme path is accepted in argument parsing
        # (We can't fully test the server without actually starting it)
        captured = capsys.readouterr()
