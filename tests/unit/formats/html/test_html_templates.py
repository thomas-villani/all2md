...
class TestReplaceTemplateMode:
    ...
    def test_custom_placeholder(self, tmp_path):
        ...
        template = tmp_path / "template.html"
        template.write_text(
            "<html><body>{{MAIN_CONTENT}}</body></html>",
            encoding='utf-8'
        )
    ...
    def test_toc_placeholder(self, tmp_path):
        ...
        template = tmp_path / "template.html"
        template.write_text(
            "<html><body><nav>{TOC}</nav>{CONTENT}</body></html>",
            encoding='utf-8'
        )
...
class TestInjectTemplateMode:
    ...
    def test_inject_replace(self, tmp_path):
        ...
        template.write_text(
            "<html><body><div id='content'><p>Old content</p></div></body></html>",
            encoding='utf-8'
        )

        doc = Document(children=[
            Paragraph(content=[Text(content="New content")])
        ])
    ...
    def test_inject_append(self, tmp_path):
        ...
        template = tmp_path / "layout.html"
        template.write_text(
            "<html><body><div id='main'><p>Existing</p></div></body></html>",
            encoding='utf-8'
        )

        doc = Document(children=[
            Paragraph(content=[Text(content="Appended")])
        ])
    ...
    def test_inject_prepend(self, tmp_path):
        ...
        template = tmp_path / "layout.html"
        template.write_text(
            "<html><body><div class='container'><p>Last</p></div></body></html>",
            encoding='utf-8'
        )
...
class TestJinjaTemplateMode:
    ...
    def test_jinja_with_metadata(self, tmp_path):
        ...
        doc = Document(
            metadata={'author': 'Jane Smith', 'custom': 'value'},
...
class TestCssClassMapping:
    ...
    def test_heading_custom_class(self):
        ...
        options = HtmlRendererOptions(
            standalone=False,
            css_class_map={'Heading': 'custom-heading'}
        )
        renderer = HtmlRenderer(options)
        result = renderer.render_to_string(doc)
    ...
    def test_paragraph_multiple_classes(self):
        ...
        options = HtmlRendererOptions(
            standalone=False,
            css_class_map={'Paragraph': ['prose', 'text-lg']}
        )
        renderer = HtmlRenderer(options)
        result = renderer.render_to_string(doc)
    ...
    def test_code_block_with_language_and_custom_class(self):
        ...
        options = HtmlRendererOptions(
            ...
            css_class_map={'CodeBlock': 'code-snippet'}
        )
        renderer = HtmlRenderer(options)
        result = renderer.render_to_string(doc)
    ...
    def test_table_custom_class(self):
        ...
        options = HtmlRendererOptions(
            standalone=False,
            css_class_map={'Table': 'data-table'}
        )
        renderer = HtmlRenderer(options)
        result = renderer.render_to_string(doc)
    ...
    def test_blockquote_custom_class(self):
        ...
        doc = Document(children=[
            BlockQuote(children=[
                Paragraph(content=[Text(content="Quote")])
            ])
        ])

        options = HtmlRendererOptions(
            standalone=False,
            css_class_map={'BlockQuote': 'quote'}
        )
        renderer = HtmlRenderer(options)
        result = renderer.render_to_string(doc)
...
class TestTemplateValidation:
    ...
    def test_standalone_ignored_with_template_mode(self, tmp_path):
        ...
        options = HtmlRendererOptions(
            standalone=True,  # Should be ignored
            template_mode='replace',
            template_file=str(template)
        )
        renderer = HtmlRenderer(options)
