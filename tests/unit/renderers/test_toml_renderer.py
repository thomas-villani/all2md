...
#
# tests/unit/renderers/test_toml_renderer.py
"""Unit tests for TOML rendering from AST.

Tests cover:
- Basic TOML rendering
- Table extraction
- List extraction
- Type inference
- Heading keys
...
from all2md.renderers.toml import DataExtractor, TomlRenderer
...
@pytest.mark.unit
class TestTomlBasicRendering:
    """Tests for basic TOML rendering functionality."""

    def test_render_simple_table(self) -> None:
        """Test rendering a simple table to TOML."""
...
class TestTomlTableExtraction:
    ...
    def test_extract_table_with_heading_key(self) -> None:
        ...
        options = TomlRendererOptions(table_heading_keys=True)
        renderer = TomlRenderer(options)
        result = renderer.render_to_string(doc)

        assert "Config" in result

    def test_extract_table_without_heading_key(self) -> None:
        """Test table extraction without heading key."""
        # Create document with table but no heading
        table = Table(
            header=TableRow(cells=[TableCell(content=[Text(content="name")])]),
            rows=[TableRow(cells=[TableCell(content=[Text(content="test")])])],
        )
        doc = Document(children=[table])
        options = TomlRendererOptions(table_heading_keys=False)
        renderer = TomlRenderer(options)
        result = renderer.render_to_string(doc)
    ...
    def test_extract_mode_tables_only(self) -> None:
        ...
        # Create document with both table and list
        table = Table(
        ...
        options = TomlRendererOptions(extract_mode="tables")
        renderer = TomlRenderer(options)
        result = renderer.render_to_string(doc)
...
class TestTomlListExtraction:
    ...
    def test_extract_mode_lists_only(self) -> None:
        ...
        options = TomlRendererOptions(extract_mode="lists")
        renderer = TomlRenderer(options)
        result = renderer.render_to_string(doc)
...
class TestTomlTypeInference:
    ...
    def test_infer_integer(self) -> None:
        ...
        assert "port = 8080" in result  # Should be integer, not string
...
class TestTomlDataExtractor:
    ...
    def test_extractor_handles_duplicate_headings(self) -> None:
        ...
        extractor = DataExtractor(TomlRendererOptions())
