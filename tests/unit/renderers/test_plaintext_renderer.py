...
class TestFormattingStripping:
    ...
    def test_strip_italic(self) -> None:
        ...
        doc = Document(
            children=[
                Paragraph(
                    content=[Text(content="Normal "), Emphasis(content=[Text(content="italic")]), Text(content=" text")]
                )
            ]
        )
        renderer = PlainTextRenderer()
    ...
    def test_nested_formatting(self) -> None:
        ...
        doc = Document(
            children=[Paragraph(content=[Strong(content=[Emphasis(content=[Text(content="bold and italic")])])])]
        )
        renderer = PlainTextRenderer()
        result = renderer.render_to_string(doc)
...
class TestHeadings:
    ...
    def test_heading_with_formatting(self) -> None:
        ...
        doc = Document(
            children=[Heading(level=2, content=[Text(content="Section with "), Strong(content=[Text(content="bold")])])]
        )
        renderer = PlainTextRenderer()
        result = renderer.render_to_string(doc)
            ...
            children=[
                Table(
                    ...
                    rows=[
                        ...
                        TableRow(
                            cells=[TableCell(content=[Text(content="Bob")]), TableCell(content=[Text(content="25")])]
                        ),
                    ],
                )
            ]
            ...
            children=[
                Table(
                    rows=[
                        TableRow(
                            cells=[
                                TableCell(content=[Strong(content=[Text(content="Bold")])]),
                                TableCell(content=[Emphasis(content=[Text(content="Italic")])]),
                            ]
                        )
                    ]
                )
...
class TestCodeBlocks:
    ...
    def test_code_block_preserved(self) -> None:
        ...
        code = """def hello():
    print("world")"""
        doc = Document(children=[CodeBlock(content=code, language="python")])
        renderer = PlainTextRenderer()
        result = renderer.render_to_string(doc)
    ...
    def test_inline_code(self) -> None:
        ...
        doc = Document(
            children=[Paragraph(content=[Text(content="Use "), Code(content="print()"), Text(content=" here")])]
        )
        renderer = PlainTextRenderer()
        result = renderer.render_to_string(doc)
...
class TestLinks:
    ...
    def test_link_text_only(self) -> None:
        ...
        doc = Document(
            children=[
                Paragraph(
                    content=[Text(content="Visit "), Link(content=[Text(content="example")], url="https://example.com")]
                )
            ]
        )
        renderer = PlainTextRenderer()
...
class TestImages:
    ...
    def test_image_alt_text(self) -> None:
        ...
        doc = Document(children=[Paragraph(content=[Image(alt_text="Photo", url="photo.jpg")])])
        renderer = PlainTextRenderer()
        result = renderer.render_to_string(doc)
        assert result == "Photo"
    ...
    def test_image_without_alt_text(self) -> None:
        ...
        doc = Document(
            children=[
                Paragraph(
                    content=[Text(content="Before "), Image(alt_text="", url="photo.jpg"), Text(content=" after")]
                )
            ]
        )
        renderer = PlainTextRenderer()
...
class TestLineBreaks:
    ...
    def test_soft_line_break(self) -> None:
        ...
        doc = Document(
            children=[Paragraph(content=[Text(content="First"), LineBreak(soft=True), Text(content="Second")])]
        )
        renderer = PlainTextRenderer()
        result = renderer.render_to_string(doc)
    ...
    def test_hard_line_break(self) -> None:
        ...
        doc = Document(
            children=[Paragraph(content=[Text(content="First"), LineBreak(soft=False), Text(content="Second")])]
        )
        renderer = PlainTextRenderer()
        result = renderer.render_to_string(doc)
        assert result == "First\nSecond"
...
class TestSkippedElements:
    ...
    def test_html_inline_skipped(self) -> None:
        ...
        doc = Document(
            children=[
                Paragraph(
                    content=[Text(content="Before "), HTMLInline(content="<span>HTML</span>"), Text(content=" after")]
                )
            ]
        )
        renderer = PlainTextRenderer()
...
class TestMath:
    ...
    def test_inline_math(self) -> None:
        ...
        doc = Document(
            children=[
                Paragraph(content=[Text(content="Formula: "), MathInline(content="x^2 + y^2 = r^2", notation="latex")])
            ]
        )
        renderer = PlainTextRenderer()
        result = renderer.render_to_string(doc)
    ...
    def test_math_block(self) -> None:
        ...
        doc = Document(children=[MathBlock(content="\\int_{0}^{\\infty} e^{-x} dx = 1", notation="latex")])
        renderer = PlainTextRenderer()
        result = renderer.render_to_string(doc)
...
class TestComplexStructures:
    ...
    def test_complex_document(self) -> None:
        ...
        doc = Document(
            children=[
                Heading(level=1, content=[Text(content="Title with "), Strong(content=[Text(content="bold")])]),
                Paragraph(
                    content=[
